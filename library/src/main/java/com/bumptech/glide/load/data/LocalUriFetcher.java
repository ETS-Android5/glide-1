...
/**
 * A DataFetcher that uses an {@link android.content.ContentResolver} to load data from a {@link android.net.Uri} pointing to a local resource.
 *
 * @param <T> The type of data that will obtained for the given uri (For example, {@link java.io.InputStream} or {@link android.os.ParcelFileDescriptor}.
 */
public abstract class LocalUriFetcher<T> implements DataFetcher<T> {
     ...
     * @param uri             A Uri pointing to a local asset. This load will fail if the uri isn't openable by {@link ContentResolver#openInputStream(android.net.Uri)}
    ...
    /**
     * Returns a concrete data type from the given {@link android.net.Uri} using the given {@link android.content.ContentResolver}.
     */
