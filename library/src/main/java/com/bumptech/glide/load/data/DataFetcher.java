 ...
 * com.bumptech.glide.load.model.ModelLoader}. {@link #loadData(com.bumptech.glide.Priority, com.bumptech.glide.load.data.DataFetcher.DataCallback)} may or may not be called for any
 * given load depending on whether or not the corresponding resource is cached. Cancel also may or may not be called. If {@link #loadData(com.bumptech.glide.Priority,
 * com.bumptech.glide.load.data.DataFetcher.DataCallback)}} is called, then so {@link #cleanup()} will be called.
 *
...
public interface DataFetcher<T> {
    ...
    * here. Any third party libraries called must be thread safe (or move the work to another thread) since this method will be called from a thread in a {@link
   ...
   /**
    * Cleanup or recycle any resources used by this data fetcher. This method will be called in a finally block after the data provided by {@link #loadData(com.bumptech.glide.Priority,
    * com.bumptech.glide.load.data.DataFetcher.DataCallback)} has been decoded by the {@link com.bumptech.glide.load.ResourceDecoder}.
    *
    * <p>Note - this method will be run on a background thread so blocking I/O is safe.
    */
   void cleanup();
   ...
   /**
    * A method that will be called when a load is no longer relevant and has been cancelled. This method does not need to guarantee that any in process loads do not finish. It also may
    * be called before a load starts or after it finishes.
    *
    * <p>The best way to use this method is to cancel any loads that have not yet started, but allow
    * those that are in process to finish since its we typically will want to display the same resource in a different view in the near future.
    *
