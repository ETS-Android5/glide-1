...
/**
 * A class that allows {@link com.bumptech.glide.load.resource.transcode.ResourceTranscoder}s to be registered and retrieved by the classes they convert between.
 */
public class TranscoderRegistry {
   private final List<Entry<?, ?>> transcoders = new ArrayList<>();

   /**
    * Registers the given {@link com.bumptech.glide.load.resource.transcode.ResourceTranscoder} using the given classes so it can later be retrieved using the given classes.
    *
   ...
   /**
    * Returns the currently registered {@link com.bumptech.glide.load.resource.transcode.ResourceTranscoder} for the given classes.
    *
   ...
   private static final class Entry<Z, R> {
      ...
      /**
       * If we convert from a specific Drawable, we must get that specific Drawable class or a subclass of that Drawable. In contrast, if we we convert <em>to</em> a specific Drawable,
       * we can fulfill requests for a more generic parent class (like Drawable). As a result, we check fromClass and toClass in different orders.
       */
