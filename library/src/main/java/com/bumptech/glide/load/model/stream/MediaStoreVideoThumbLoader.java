...
/**
 * Loads {@link InputStream}s from media store video {@link Uri}s that point to pre-generated thumbnails for those {@link Uri}s in the media store.
 *
 * <p>If {@link VideoDecoder#TARGET_FRAME} is set with a non-null value that is not equal to {@link
 * VideoDecoder#DEFAULT_FRAME}, this loader will always return {@code null}. The media store does not use a defined frame to generate the thumbnail, so we cannot accurately fulfill requests for specific frames.
 */
public class MediaStoreVideoThumbLoader implements ModelLoader<Uri, InputStream> {
    ...
    /**
     * Loads {@link InputStream}s from media store image {@link Uri}s that point to pre-generated thumbnails for those {@link Uri}s in the media store.
     */
