 ...
 *
 * @param <T> The type of the option ({@link Integer}, {@link android.graphics.Bitmap.CompressFormat} etc.), must implement {@link #equals(Object)} and {@link #hashCode()}.
 */
public final class Option<T> {
   ...
   /**
    * Returns a new {@link Option} that does not affect disk cache keys with the given value as the default value.
    *
    * @param key A unique package prefixed {@link String} that identifies this option (must be stable across builds, so {@link Class#getName()} should <em>not</em> be used).
    */
   @NonNull
   ...
   /**
    * Returns a new {@link Option} that uses the given {@link com.bumptech.glide.load.Option.CacheKeyUpdater} to update disk cache keys.
    *
    * @param key A unique package prefixed {@link String} that identifies this option (must be stable across builds, so {@link Class#getName()} should <em>not</em> be used).
    */
   @NonNull
   ...
   /**
    * An interface that updates a {@link MessageDigest} with the given value as part of a process to generate a disk cache key.
    *
    * @param <T> The type of the option.
    */
   public interface CacheKeyUpdater<T> {
      /**
       * Updates the given {@link MessageDigest} with the bytes of the given key (to avoid incidental value collisions when values are not particularly unique) and value.
       *
