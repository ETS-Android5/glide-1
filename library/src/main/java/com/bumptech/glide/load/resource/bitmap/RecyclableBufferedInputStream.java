...
public class RecyclableBufferedInputStream extends FilterInputStream {
   ...
   /**
    * The currently marked position. -1 indicates no mark has been put or the mark has been invalidated.
    */
    ...
    * the stream back to the marked position if {@code readlimit} has not been surpassed. The underlying buffer may be increased in size to allow {@code readlimit} number of bytes to be
    * supported.
    *
   ...
   /**
    * Indicates whether {@code BufferedInputStream} supports the {@link #mark(int)} and {@link #reset()} methods.
    *
   ...
   /**
    * Reads at most {@code byteCount} bytes from this stream and stores them in byte array {@code buffer} starting at offset {@code offset}. Returns the number of bytes actually read or
    * -1 if no bytes were read and the end of the stream was encountered. If all the buffered bytes have been used, a mark has not been put and the requested number of bytes is larger
    * than the receiver's buffer size, this implementation bypasses the buffer and simply places the results directly into {@code buffer}.
    *
    * @param buffer the byte array in which to store the bytes read.
    * @return the number of bytes actually read or -1 if end of stream.
    * @throws IndexOutOfBoundsException if {@code offset < 0} or {@code byteCount < 0}, or if {@code offset + byteCount} is greater than the size of {@code buffer}.
   ...
   /**
    * Skips {@code byteCount} bytes in this stream. Subsequent calls to {@link #read} will not return these bytes unless {@link #reset} is used.
    *
    * @param byteCount the number of bytes to skip. This method does nothing and returns 0 if {@code byteCount} is less than zero.
