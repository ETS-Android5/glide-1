...
/**
 * A singleton to present a simple static interface for building requests with {@link RequestBuilder} and maintaining an {@link Engine}, {@link BitmapPool}, {@link com.bumptech.glide.load.engine.cache.DiskCache} and {@link MemoryCache}.
 */
public class Glide implements ComponentCallbacks2 {
     ...
     * Any {@link android.graphics.Bitmap} added to the pool must not be currently in use in any other part of the application. Any {@link android.graphics.Bitmap} added to the pool must be removed from the pool before it is added a second time.
     *
     ...
     * currently in the pool will be evicted. Bitmaps are allocated according to the weights of the given sizes, where each size gets (weight / prefillWeightSum) percent of the pool to fill.
     *
     ...
     * worse than not pre-filling at all. Pre-filling should only be started in onCreate to avoid constantly clearing and re-filling the {@link BitmapPool}. Rotation should be carefully considered as well. It may be worth calling this method only when no saved instance state exists so that
     ...
     * MemoryCategory#HIGH} increases Glide's maximum memory usage by up to 50% and {@link MemoryCategory#LOW} decreases Glide's maximum memory usage by 50%. This method should be used to temporarily increase or decrease memory usage for a single Activity or part of the app. Use {@link
     * GlideBuilder#setMemoryCache(MemoryCache)} to put a permanent memory size if you want to change the default.
     *
     ...
     * and will not be started or stopped based on lifecycle events. In general, loads should be started at the level the result will be used in. If the resource will be used in a view in a child fragment, the load should be started with {@link #with(android.app.Fragment)}} using that child
     * fragment. Similarly, if the resource will be used in a view in the parent fragment, the load should be started with {@link #with(android.app.Fragment)} using the parent fragment. In the same vein, if the resource will be used in a view in an activity, the load should be started with {@link
    ...
    /**
     * Begin a load with Glide that will tied to the give {@link androidx.fragment.app.FragmentActivity}'s lifecycle and that uses the given {@link androidx.fragment.app.FragmentActivity}'s default options.
     *
     * @param activity The activity to use.
    ...
    /**
     * Begin a load with Glide that will be tied to the given {@link androidx.fragment.app.Fragment}'s lifecycle and that uses the given {@link androidx.fragment.app.Fragment}'s default options.
     *
