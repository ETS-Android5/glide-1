...
/**
 * A singleton to present a simple static interface for building requests with {@link RequestBuilder} and maintaining an {@link Engine}, {@link BitmapPool}, {@link
...
public class Glide implements ComponentCallbacks2 {
     ...
     */
    @Important("0.currentMaxIndex:20")
    @NonNull
     ...
     * versions of Android if you try to decode a hardware Bitmap. This method is only useful for testing.
     */
    @VisibleForTesting
    ...
    /**
     * Returns the {@link com.bumptech.glide.load.engine.bitmap_recycle.BitmapPool} used to temporarily store {@link android.graphics.Bitmap}s so they can be reused to avoid garbage
     * collections.
     *
     * <p>Note - Using this pool directly can lead to undefined behavior and strange drawing errors.
     * Any {@link android.graphics.Bitmap} added to the pool must not be currently in use in any other part of the application. Any {@link android.graphics.Bitmap} added to the pool
     * must be removed from the pool before it is added a second time.
     *
     * <p>Note - To make effective use of the pool, any {@link android.graphics.Bitmap} removed from
     * the pool must eventually be re-added. Otherwise the pool will eventually empty and will not serve any useful purpose.
     *
     * <p>The primary reason this object is exposed is for use in custom {@link
     * com.bumptech.glide.load.ResourceDecoder}s and {@link com.bumptech.glide.load.Transformation}s. Use outside of these classes is not generally recommended.
     */
    @NonNull
     ...
     * currently in the pool will be evicted. Bitmaps are allocated according to the weights of the given sizes, where each size gets (weight / prefillWeightSum) percent of the pool to
     * fill.
     *
     ...
     * worse than not pre-filling at all. Pre-filling should only be started in onCreate to avoid constantly clearing and re-filling the {@link BitmapPool}. Rotation should be carefully
     * considered as well. It may be worth calling this method only when no saved instance state exists so that pre-filling only happens when the Activity is first created, rather than
     * on every rotation.
     *
     * @param bitmapAttributeBuilders The list of {@link Builder Builders} representing individual sizes and configurations of {@link Bitmap}s to be pre-filled.
     */
     ...
     * MemoryCategory#HIGH} increases Glide's maximum memory usage by up to 50% and {@link MemoryCategory#LOW} decreases Glide's maximum memory usage by 50%. This method should be used
     * to temporarily increase or decrease memory usage for a single Activity or part of the app. Use {@link GlideBuilder#setMemoryCache(MemoryCache)} to put a permanent memory size if
     ...
     * and will not be started or stopped based on lifecycle events. In general, loads should be started at the level the result will be used in. If the resource will be used in a view
     * in a child fragment, the load should be started with {@link #with(android.app.Fragment)}} using that child fragment. Similarly, if the resource will be used in a view in the
     * parent fragment, the load should be started with {@link #with(android.app.Fragment)} using the parent fragment. In the same vein, if the resource will be used in a view in an
     * activity, the load should be started with {@link #with(android.app.Activity)}}.
     *
    ...
    /**
     * Begin a load with Glide that will tied to the give {@link androidx.fragment.app.FragmentActivity}'s lifecycle and that uses the given {@link
    ...
    /**
     * Begin a load with Glide that will be tied to the given {@link androidx.fragment.app.Fragment}'s lifecycle and that uses the given {@link androidx.fragment.app.Fragment}'s default
     * options.
     *
    ...
    /**
     * ss Begin a load with Glide that will be tied to the given {@link android.app.Fragment}'s lifecycle and that uses the given {@link android.app.Fragment}'s default options.
     *
     * @param fragment The fragment to use.
     * @return A RequestManager for the given Fragment that can be used to start a load.
     * @deprecated Prefer support Fragments and {@link #with(Fragment)} instead, {@link android.app.Fragment} will be deprecated. See
    ...
    /**
     * Begin a load with Glide that will be tied to the lifecycle of the {@link Fragment}, {@link android.app.Fragment}, or {@link Activity} that contains the View.
     *
     ...
     * Consider memoizing the result after the View is attached or again, prefer the Activity and Fragment variants whenever possible.
     *
     * <p>When used in Applications that use the non-support {@link android.app.Fragment} classes,
     * calling this method will produce noisy logs from {@link android.app.FragmentManager}. Consider avoiding entirely or using the {@link Fragment}s from the support library instead.
     *
     * <p>If the support {@link FragmentActivity} class is used, this method will only attempt to
     * discover support {@link Fragment}s. Any non-support {@link android.app.Fragment}s attached to the {@link FragmentActivity} will be ignored.
     *
