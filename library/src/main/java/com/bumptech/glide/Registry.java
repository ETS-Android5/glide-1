...
public class Registry {
   ...
   /**
    * Overrides the default ordering of resource decoder buckets. You may also add custom buckets which are identified as a unique string. Glide will attempt to decode using decoders in
    * the highest priority bucket before moving on to the next one.
    *
   ...
   /**
    * Registers a new {@link com.bumptech.glide.load.data.DataRewinder.Factory} to handle a non-default data type that can be rewind to allow for efficient reads of file headers.
    */
   @NonNull
   ...
   /**
    * Registers the given {@link ResourceTranscoder} to convert from the given resource {@link Class} to the given transcode {@link Class}.
    *
    * @param resourceClass  The class that will be transcoded from (e.g. {@link android.graphics.Bitmap}).
    * @param transcodeClass The class that will be transcoded to (e.g. {@link android.graphics.drawable.BitmapDrawable}).
   ...
   /**
    * Registers a new {@link ImageHeaderParser} that can obtain some basic metadata from an image header (orientation, type etc).
    */
   @NonNull
