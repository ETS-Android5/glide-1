...
public final class SingleRequest<R> implements Request, SizeReadyCallback, ResourceCallback {
    ...
    @Important("19.SingleRequest中的width指的是该request最终输出的bitmap宽度")
    @GuardedBy("requestLock")
    private int width;
    ...
    public void begin() {
        synchronized (requestLock) {
            ...
            if (status == Status.COMPLETE) {
                Log.i("SingleRequest", "onResourceReady from previous");
    ...
    /**
     * Cancels the current load but does not release any resources held by the request and continues to display the loaded resource if the load completed before the call to cancel.
     *
    ...
    public void onSizeReady(int width, int height) {
        ...
        synchronized (requestLock) {
            ...
            Log.i("SingleRequest","model:" + model.getClass().getSimpleName() + " resourceClass:" + requestOptions.getResourceClass().getSimpleName() + " transcodeClass:" + transcodeClass.getSimpleName());
            loadStatus =
                    engine.load(
                            glideContext,
                            model,
    ...
    /**
     * Internal {@link #onResourceReady(Resource, DataSource, boolean)} where arguments are known to be safe.
     *
