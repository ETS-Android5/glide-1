 ...
 * RequestListener} is added to a request that is started with {@link RequestBuilder#submit()}, {@link RequestBuilder#submit(int, int)}, or {@link RequestBuilder#into(int, int)}. Those methods no longer post results back to the main thread to avoid the unnecessary thread interactions and
 * corresponding latency. As a side affect though, listeners added to those requests are no longer called on the main thread. {@code RequestListeners} added to requests started with {@link RequestBuilder#into(Target)} or {@link RequestBuilder#into(ImageView)} will continue to be called back on the
 * main thread.
 *
...
public interface RequestListener<R> {
    ...
    /**
     * Called when an exception occurs during a load, immediately before {@link Target#onLoadFailed(Drawable)}. Will only be called if we currently want to display an image for the given model in the given target. It is recommended to create a single instance per activity/fragment rather than
     * instantiate a new object for each call to {@code Glide.with(fragment/activity).load()} to avoid object churn.
     *
     ...
     * @param isFirstResource {@code true} if this is the first resource to in this load to be loaded into the target. For example when loading a thumbnail and a full-sized image, this will be {@code true} for the first image to load and {@code false} for the second.
