...
/**
 * A base object to allow method sharing between {@link RequestOptions} and {@link com.bumptech.glide.RequestBuilder}.
 *
...
public abstract class BaseRequestOptions<T extends BaseRequestOptions<T>> implements Cloneable {
    ...
    * Glide's threads as part of another request. Using this method in other scenarios can lead to excessive memory usage and OOMs and/or a significant decrease in performance across an
    * application.
    *
   ...
   /**
    * Sets an {@link Drawable} to display if the model provided to {@link com.bumptech.glide.RequestBuilder#load(Object)} is {@code null}.
    *
   ...
   /**
    * Sets a resource to display if the model provided to {@link com.bumptech.glide.RequestBuilder#load(Object)} is {@code null}.
    *
    * <p>If a fallback is not set, null models will cause the error drawable to be displayed. If the
    * error drawable is not set, the placeholder will be displayed.
    *
    ...
    *
    * @param signature A unique non-null {@link Key} representing the current state of the model that will be mixed in to the cache key.
    * @return This request builder.
    ...
    *
    * @param frameTimeMicros The time position in microseconds of the desired frame. If negative, the Android framework implementation return a representative frame.
    ...
    * decoded using a decoder that cannot control the format ({@link android.media.MediaMetadataRetriever} for example), or that the decoder may choose to ignore the requested format if
    * it can't display the image (i.e. RGB_565 is requested, but the image has alpha).
    *
    ...
    * and {@link Downsampler#DECODE_FORMAT}. To set only the format used when decoding GIF frames, use {@link #set(Option, Object)} and {@link GifOptions#DECODE_FORMAT}.
    *
   ...
   /**
    * Sets the {@link DownsampleStrategy} to use when decoding {@link Bitmap Bitmaps} using {@link Downsampler}.
    *
   ...
   /**
    * Applies {@link com.bumptech.glide.load.resource.bitmap.CenterCrop} to all default types, and ignores unknown types.
    *
   ...
   /**
    * Applies {@link CenterCrop} to all default types and throws an exception if asked to transform an unknown type.
    *
    * <p>this will override previous calls to {@link #dontTransform()} ()}.
    *
   ...
   /**
    * Applies {@link FitCenter} and to all default types, {@link DownsampleStrategy#FIT_CENTER} to image types, and ignores unknown types.
    *
   ...
   /**
    * Applies {@link FitCenter} and to all default types, {@link DownsampleStrategy#FIT_CENTER} to image types, and throws an exception if asked to transform an unknown type.
    *
    * <p>This will override previous calls to {@link #dontTransform()} and previous calls to {@link
    * #downsample(DownsampleStrategy)}.
    *
   ...
   /**
    * Applies {@link com.bumptech.glide.load.resource.bitmap.CenterInside} to all default types, {@link DownsampleStrategy#CENTER_INSIDE} to image types, and ignores unknown types.
    *
    * <p>This will override previous calls to {@link #dontTransform()} and previous calls to {@link
    * #downsample(DownsampleStrategy)}.
    *
   ...
   /**
    * Applies {@link CenterInside} to all default types, {@link DownsampleStrategy#CENTER_INSIDE} to image types and throws an exception if asked to transform an unknown type.
    *
    * <p>This will override previous calls to {@link #dontTransform()} and previous calls to {@link
    * #downsample(DownsampleStrategy)}.
    *
   ...
   /**
    * Applies {@link CircleCrop} to all default types and throws an exception if asked to transform an unknown type.
    *
    * <p>This will override previous calls to {@link #dontTransform()}.
    *
    * @see #transform(Class, Transformation)
   ...
   /**
    * Applies the given {@link Transformation}s in the given order for {@link Bitmap Bitmaps} to the default types ({@link Bitmap}, {@link android.graphics.drawable.BitmapDrawable}, and
    * {@link com.bumptech.glide.load.resource.gif.GifDrawable}) and throws an exception if asked to transform an unknown type.
    *
    * <p>This will override previous calls to {@link #dontTransform()}.
    *
   ...
   /**
    * Applies the given {@link Transformation} for any decoded resource of the given type and allows unknown resource types to be ignored.
    *
    * <p>Users can apply different transformations for each resource class. Applying a {@link
    * Transformation} for a resource type that already has a {@link Transformation} will override the previous call.
    *
    * <p>If any calls are made to the non-optional transform methods, then attempting to transform an
    * unknown resource class will throw an exception. To allow unknown types, users must always call the optional version of each method.
    *
    * <p>This will override previous calls to {@link #dontTransform()}.
    *
   ...
   /**
    * Applies the given {@link Transformation} for any decoded resource of the given type and throws if asked to transform an unknown resource type.
    *
    * <p>This will override previous calls to {@link #dontTransform()}.
    *
    * @param resourceClass  The type of resource to transform.
   ...
   /**
    * Removes all applied {@link Transformation Transformations} for all resource classes and allows unknown resource types to be transformed without throwing an exception.
    */
   @NonNull
   @CheckResult
