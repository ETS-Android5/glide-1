...
public abstract class ViewTarget<T extends View, Z> extends BaseTarget<Z> {
    ...
    *                      forces Glide to wait for the layout pass to occur before starting the load, setting this parameter to {@code true} can cause flashing in some cases and should
    *                      be used sparingly. If layout parameters are set to fixed sizes, they will still be used instead of the View's dimensions even if this parameter is set to
    *                      {@code true}. This parameter is a fallback only.
    ...
    * transitioning screens or swapping adapters in scrolling views. However it also substantially increases the odds that images will not be in memory if users subsequently return to a
    * screen where images were previously loaded. Whether or not this happens will depend on the number of images loaded in the new screen and the size of the memory cache. Increasing
    * the size of the memory cache can improve this behavior but it largely negates the memory benefits of using this method.
    *
   ...
   /**
    * Indicates that Glide should always wait for any pending layout pass before checking for the size an {@link View}.
    *
    ...
    * Android 4.0 may cause memory leaks and isn't recommended. If you do use this method on older versions, be sure to call {@link com.bumptech.glide.RequestManager#clear(View)} on any
    * view you start a load into to ensure that the static state is removed.
    *
   ...
   static final class SizeDeterminer {
      ...
      /**
       * The callback may be called anyway if it is removed by another {@link SizeReadyCallback} or otherwise removed while we're notifying the list of callbacks.
       *
       * <p>See #2237.
       */
