 ...
 *
 * @param <TranscodeType> The type of resource that will be delivered to the {@link com.bumptech.glide.request.target.Target}.
 */
// Public API.
...
public class RequestBuilder<TranscodeType> extends BaseRequestOptions<RequestBuilder<TranscodeType>> implements Cloneable, ModelTypes<RequestBuilder<TranscodeType>> {
   ...
   /**
    * Sets the {@link TransitionOptions} to use to transition from the placeholder or thumbnail when this load completes.
    *
   ...
   /**
    * Sets a {@link RequestListener} to monitor the resource load and removes all previously set listeners (either via this method or from {@link #addListener(RequestListener)} .
    *
   ...
   /**
    * Adds a {@link RequestListener} to the list that will be called in the order they were added when the request ends.
    *
    ...
    * request in the chain. For example, if you start a primary request with a single nested thumbnail and you add a {@link RequestListener} only to the primary request, then the {@link
    * RequestListener} will only be notified when the primary request succeeds or fails. If the thumbnail succeeds, but the primary request fails, the {@link RequestListener} added to
    * the primary request will still be called with {@link RequestListener#onLoadFailed(GlideException, Object, Target, boolean)}. In the same scenario, the {@link RequestListener}
    * added only to the primary request will not have {@link RequestListener#onResourceReady(Object, Object, Target, DataSource, boolean)} called when the thumbnail request finishes
    * successfully. Similarly, if you add a {@link RequestListener} only to a thumbnail request, but not the primary request, that {@code listener} will only be called for changes
    * related to the thumbnail request. If the thumbnail request fails, the {@code listener} added to the thumbnail request will be immediately called via {@link
    ...
    * mind that if any parent request finishes before its thumbnail request(s), it will attempt to cancel those requests. As a result there's no guarantee that a {@link RequestListener}
    * added to a thumbnail request will actually be called with either success or failure. These same patterns hold for arbitrarily nested thumbnails. The {@code listener} is only
    * called for the requests it is added to and may not be called for every thumbnail request if those requests are cancelled due to the completion of a parent request.
    *
    ...
    * starting. {@link Target#onLoadFailed(Drawable)} will only be called if every request in the chain, including the primary request, fails, which you can use to trigger your fallback
    * behavior. Be sure to pick an appropriate {@link Target} subclass when possible, like {@link com.bumptech.glide.request.target.BitmapImageViewTarget} or {@link
   ...
   /**
    * Identical to calling {@link #error(RequestBuilder)} where the {@code RequestBuilder} is the result of calling {@link #clone()} and removing any existing thumbnail and error {@code
    * RequestBuilders}.
    *
   ...
   /**
    * Loads and displays the resource retrieved by the given thumbnail request if it finishes before this request. Best used for loading thumbnail resources that are smaller and will be
    * loaded more quickly than the full size resource. There are no guarantees about the order in which the requests will actually finish. However, if the thumb request completes after
    * the full request, the thumb resource will never replace the full resource.
    *
    ...
    * the javadoc on {@link #listener(RequestListener)} for one concrete example of the behavior differences and complexity introduced by this method. Better consistency and readability
    * can be obtained by calling {@link #thumbnail(RequestBuilder)} with a duplicate {@code RequestBuilder} on which you have called {@link BaseRequestOptions#sizeMultiplier(float)}. In
    * practice this method also isn't especially useful. It's much more common to want to specify a number of different attributes for thumbnails than just a simple percentage modifier
    * on the target size, so there's little justification for keeping this method. This method will be removed in a future version of Glide.
    */
   @NonNull
   @CheckResult
   @SuppressWarnings("unchecked")
    ...
    * com.bumptech.glide.request.RequestOptions#signature(com.bumptech.glide.load.Key)} to mixin a signature you create based on the data at the given Uri that will invalidate the cache
    * if that data changes. Alternatively, using {@link com.bumptech.glide.load.engine.DiskCacheStrategy#NONE} and/or {@link com.bumptech.glide.request.RequestOptions#skipMemoryCache(boolean)}
    * may be appropriate.
    *
   ...
   /**
    * Returns a request builder that uses the {@link com.bumptech.glide.load.model.ModelLoaderFactory} currently registered or {@link Integer} to load the image represented by the given
    * {@link Integer} resource id. Defaults to {@link com.bumptech.glide.load.model.ResourceLoader} to load resource id models.
    *
    ...
    * you do not increment your version code before each install and you replace a Drawable with different data without changing the Drawable name, you may see inconsistent cached data.
    * To get around this, consider using {@link com.bumptech.glide.load.engine.DiskCacheStrategy#NONE} via {@link RequestOptions#diskCacheStrategy(com.bumptech.glide.load.engine.DiskCacheStrategy)}
