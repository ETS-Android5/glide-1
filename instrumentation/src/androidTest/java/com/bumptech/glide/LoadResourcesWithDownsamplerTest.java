...
/**
 * On API 26, decoding a variety of different images can cause {@link BitmapFactory} with {@link BitmapFactory.Options#inJustDecodeBounds} set to {@code true} to set {@link BitmapFactory.Options#outConfig} to null instead of a valid value, even though the image can be decoded successfully. Glide can
 * mask these failures by decoding some image sources (notably including resource ids) using other data types and decoders.
 *
