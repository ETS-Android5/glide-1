...
public class NonBitmapDrawableResourcesTest {
    ...
    /**
     * Returns {@code true} iff the resource package name is exactly the same as the containing application package name for a given resource id.
     *
     * <p>The resource package name is the value returned by {@link
     * Resources#getResourcePackageName(int)}. The application package name is package name of the enclosing application. If these two things are equal, then we can both construct a Context for that package and retrieve a resource id for that package from a "standard" resource Uri containing a name
     * instead of an id. If they aren't equal, then we can do only one of the two required tasks, so our Uri load will always fail. To handle this properly, we'd need callers to include both package names in the Uri. I'm not aware of any standardized Uri format for doing so, so these requests will
     * just be treated as unsupported for the time being.
     *
