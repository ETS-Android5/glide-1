...
/**
 * Checks for regressions for a given Glide load by comparing the result of a load to a previously saved Bitmap.
 *
 * <p>Can be used to generate or re-generate expected {@link Bitmap}s by placing a file named
 * "regenerate" in /sdcard/DCIM/test_files. The apks containing this tester will need to have {@link android.Manifest.permission#WRITE_EXTERNAL_STORAGE}. Resources can be split by apk by adding {@link SplitBySdk} to test methods or classes. If {@link SplitBySdk} is added to both a test class and a
 * particular method, the values from the method will be used.
 *
