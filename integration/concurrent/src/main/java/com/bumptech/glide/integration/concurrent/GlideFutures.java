...
public final class GlideFutures {
    ...
    * com.google.common.util.concurrent.ClosingFuture} or occasionally failing to return resources to the pool. Because failing to return resources to the pool is inefficient, but safe,
    * that's the route we've chosen. A more sophisticated implementation may allow us to avoid the resource inefficiency.
    *
    ...
    * }</pre>
    *
    * @param requestBuilder A request builder for the resource to load. It must be tied to an application Glide instance, and must not have a listener set.
    * @param <T>            The type of resource that will be loaded (Bitmap, Drawable, etc).
    */
